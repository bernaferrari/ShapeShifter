...
export const isPrimaryButtonReleased = (event: MouseEvent | PointerEvent) => event.buttons === 0;
...
export const safelyReleasePointerCapture = (target: Element, pointerId: number) => {
  try {
...
export const bindWindowPointerDrag = ({ onMove, onEnd }: PointerDragOptions) => {
  let active = true;
...
export const bindWindowTouchMouseDrag = ({ onMove, onEnd }: TouchMouseDragOptions) => {
  let active = true;

...
export class PathMutator {
  ...
  reverseSubPath(subIdx: number) {
    ...
    this.setSubPathStateLeaf(subIdx, this.findSubPathStateLeaf(subIdx).mutate().reverse().build());
    return this;
  }
  ...
  private shift(subIdx: number, calcOffsetFn: (offset: number, numCommands: number) => number) {
    ...
    this.setSubPathStateLeaf(
      subIdx,
      sps.mutate().setShiftOffset(calcOffsetFn(sps.getShiftOffset(), numCmdsInSubPath)).build(),
    );
    return this;
  }
  ...
  convertCommand(subIdx: number, cmdIdx: number, svgChar: SvgChar) {
    ...
    this.setSubPathStateLeaf(
      ...
      this.findSubPathStateLeaf(subIdx)
        .mutate()
        .setCommandState(csIdx, targetCs.mutate().convertAtIndex(splitIdx, svgChar).build())
        .build(),
    );
    return this;
  }
  ...
  splitStrokedSubPath(subIdx: number, cmdIdx: number) {
    ...
    this.setSubPathStateLeaf(subIdx, sps.mutate().setSplitSubPaths(splitSubPaths).build());
  ...
  splitFilledSubPath(subIdx: number, startCmdIdx: number, endCmdIdx: number) {
    ...
    const siblingSplitBackingIds = _(targetSps.getCommandStates() as CommandState[])
      .filter(
        (cs) => !!cs.getSplitSegmentId() && !parentSplitBackingIds.includes(cs.getBackingId()),
      )
      .map((cs) => cs.getBackingId())
      .value();
    ...
    if (
      ...
      newStates.push(targetSps.mutate().setSplitSubPaths(splitSubPaths).build());
    } else {
  ...
  private deleteFilledSubPathSegmentInternal(subIdx: number, targetCs: CommandState) {
    ...
    const secondSplitSegId = _.last(_.last(splitCss2).getParentCommandState().getCommands()).id;
    ...
    return _(orderedSubPathCmds)
      .map((cmds, subIdx) => {
        ...
        if (subIdx === 0 && moveCmd.start) {
          cmds[0] = moveCmd.mutate().setPoints(undefined, moveCmd.end).build();
        } else if (subIdx !== 0) {
          const start = _.last(orderedSubPathCmds[subIdx - 1]).end;
          cmds[0] = moveCmd.mutate().setPoints(start, moveCmd.end).build();
        }
        return cmds;
      })
      .flatMap((cmds) => cmds)
...
function reverseCommands(subPathState: SubPathState) {
  ...
  newCmds.unshift(cmds[0].mutate().setPoints(cmds[0].start, newCmds[0].start).build());
  return newCmds;
}
...
function shiftCommands(subPathState: SubPathState, cmds: Command[]) {
  ...
  if (shiftOffset === 1) {
    newCmds.push(cmds[0].mutate().setPoints(cmds[0].start, cmds[1].end).build());
  ...
  newCmds.unshift(cmds[0].mutate().setPoints(prevMoveCmd.start, _.last(newCmds).end).build());
  return newCmds;
}
